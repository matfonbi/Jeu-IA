...
from core.map_settings_loader import MapSettingsLoader
...
class Game(arcade.Window):
    def __init__(self):
        ...
        self.map_settings = MapSettingsLoader()
    ...
    def apply_map_settings(self, map_name: str):
        settings = self.map_settings.get_settings_for(map_name)

        if settings:
            self.camera.zoom = settings.get("zoom", self.default_zoom)
            self.player.scale = settings.get("player_scale", self.default_player_scale)
            self.player_speed = settings.get("player_speed", BASE_PLAYER_SPEED)
        else:
    ...
    def on_update(self, dt):
        ...
        self.transition_system.update()      
        self.transition_system.update_fade()  

        # map transitions triggered by E
